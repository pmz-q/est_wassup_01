...
class HomeData(Dataset):
  ...
  def _X_preprocess(self, X_df: pd.DataFrame):
    ...
    enc.fit(df_cat)
    df_cat_onehot = pd.DataFrame(enc.transform(df_cat), columns=enc.get_feature_names_out())
    
    return pd.concat([df_num, df_cat_onehot, df_embed], axis=1).set_index(X_df.index)
